/**
 * DEBUG MENU — temporarily removed from App.tsx
 *
 * To restore:
 * 1. Add these imports back to App.tsx (if not already present):
 *      Bug, Zap, Trash2, Gift  — from 'lucide-react'
 *
 * 2. Add the state near the other UI state declarations (around line 154):
 *      const [isDebugOpen, setIsDebugOpen] = useState(false);
 *
 * 3. Add all the debug action functions (below the other callbacks, before `goGameWorld`):
 *    See "DEBUG ACTIONS" section below.
 *
 * 4. Replace the placeholder left-side header button with the Bug button:
 *    See "HEADER BUG BUTTON" section below.
 *
 * 5. Add the Debug Menu Overlay JSX inside the return, after the world-unlock AnimatePresence block:
 *    See "DEBUG OVERLAY JSX" section below.
 */
...
// ─── HEADER BUG BUTTON ────────────────────────────────────────────────────────
// Replaces the empty <div className="justify-self-start" /> in the header left slot.
...
// ─── DEBUG OVERLAY JSX ────────────────────────────────────────────────────────
// Insert after the world-unlock AnimatePresence block (before {/* Slime Detail Popup */}).
